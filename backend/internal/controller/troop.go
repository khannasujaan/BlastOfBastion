...
func GetTroopLevel(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Invalid Method", http.StatusMethodNotAllowed)
		return
	}
	...
	response, err := repository.GetInfoTroop(parsedUUID)
	if err == repository.ErrTownhallLevelLow {
		http.Error(w, "TownHall level low", http.StatusBadRequest)
		log.Println("Townhall level low")
		return
...
func GetTroopData(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Invalid Method", http.StatusMethodNotAllowed)
		return
	}
	var err error
	var TroopReq dto.TroopDataRequest
	err = json.NewDecoder(r.Body).Decode(&TroopReq)
	if err != nil {
		http.Error(w, "Invalid JSON", http.StatusBadRequest)
		return
	...
	response, err := repository.GetTroopData(parsedUUID, TroopReq)
	if err == repository.ErrTownhallLevelLow {
		http.Error(w, "TownHall level low", http.StatusBadRequest)
		log.Println("Townhall level low")
		return
